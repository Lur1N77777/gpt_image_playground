...
import { useStore, getCachedImage, ensureImageCached, toggleTaskFavorite, retryTask } from '../store'
...
export default function TaskCard({
  ...
  const visibleOutputCount = Math.max(task.outputImages.length, task.outputSlots?.length || 0)
  const targetOutputCount = task.status !== 'running' && visibleOutputCount > 0
    ? visibleOutputCount
    : Math.max(task.params.n || 1, task.outputImages.length, task.outputSlots?.length || 0)
  const completedOutputCount = task.outputSlots?.length
    ? task.outputSlots.filter((slot) => slot.imageId).length
    : task.outputImages.length
  const aggregateActualParams = completedOutputCount > 0 ? { ...task.actualParams, n: completedOutputCount } : task.actualParams
  ...
  }, [task.outputImages, imageCacheRevision])
              ...
              <>
                ...
                {task.outputImages.length > 0 && (
                  <span className="absolute bottom-1 right-1 bg-black/60 text-white text-xs px-1.5 py-0.5 rounded">
                    {completedOutputCount}/{targetOutputCount}
                  </span>
                )}
                {task.status === 'running' && (
                  <span className="absolute bottom-1 left-1 bg-blue-500/90 text-white text-[10px] px-1.5 py-0.5 rounded">
                    继续生成中
                  </span>
                )}
              </>
          ...
          <div className="flex-1 p-3 flex flex-col min-w-0">
            ...
            <div className="mt-auto flex flex-col gap-1.5">
              ...
              <div className="flex overflow-x-auto hide-scrollbar gap-1.5 whitespace-nowrap mask-edge-r min-w-0 pr-2">
                {task.remote && task.status === 'running' && (
                  <span className="inline-flex flex-shrink-0 items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-600 dark:bg-blue-500/10 dark:text-blue-300">
                    <span className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse" />
                    云端生成中，可关闭浏览器
                  </span>
                )}
                {currentUser?.role === 'admin' && task.username && (
                  <span className="text-xs px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-300 flex-shrink-0">
                    用户：{task.username}
                  </span>
                )}
